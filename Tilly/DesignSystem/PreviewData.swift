...
/// seeds a store anyone uses. Anchored relative to `today` so the current month always
...
enum PreviewData {
    static func insert(into context: ModelContext, today: Date, calendar: Calendar) throws {
        ...
        let rent = Expense(name: "Rent", amount: 950, anchorDate: anchor(day: 1, monthOffset: -6), category: home)
        // Skipped this month, on the same day as the rent: two charges on one day.
        let streaming = Expense(
            name: "Streaming", amount: 13, anchorDate: anchor(day: 1, monthOffset: -6), category: subscriptions
        )
        let cloudStorage = Expense(
            name: "Cloud storage", amount: 3, anchorDate: anchor(day: 5, monthOffset: -6), category: subscriptions
        )
        let electricity = Expense(
            name: "Electricity", amount: 68, anchorDate: anchor(day: 8, monthOffset: -6), category: home
        )
        let phonePlan = Expense(
            name: "Phone plan", amount: 25, anchorDate: anchor(day: 8, monthOffset: -6), category: phone
        )
        // A yearly bill anchored nine months back: it doesn't recur into the current month.
        let carInsurance = Expense(
            name: "Car insurance",
            amount: 1240,
            recurrenceUnit: .year,
            anchorDate: anchor(day: 14, monthOffset: -9),
            category: car
        )
        ...
        let water = Expense(
            ...
            anchorDate: anchor(day: 22, monthOffset: -6),
            category: home
        )
        let gym = Expense(name: "Gym", amount: 40, anchorDate: anchor(day: 28, monthOffset: -6), category: health)
